...
def getLastUpdate(req, offset=None):
    ...
    response = requests.get(updates_url, timeout=10).json()
...
def addressQuery(update):
    ...
    if getUserID(update) not in userid_answers_dict:
        sendMessage(
            getChatID(update),
    ...
    if command == "invalid":
        ...
        """
        userid_answers_dict[getUserID(update)] = giveOneAnswer(
            getChatID(update), userid_answers_dict[getUserID(update)]
        )

        if len(userid_answers_dict[getUserID(update)]) <= 1:
            sendMessage(
...
def voteOnAnswer(chatID, command, update):  # upvotes or downvotes on answers
    ...
    newvalues = {
        "$set": {
            "upvotes": answer_obj["upvotes"],
            "downvotes": answer_obj["downvotes"],
